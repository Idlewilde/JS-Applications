...
function createMovieCard(movie, user, likes, ownLike) {
    ...
    let deleteBTN = element.getElementsByClassName('btn btn-danger')[0];
    let editBTN = element.getElementsByClassName('btn btn-warning')[0];
    if(deleteBTN){
    deleteBTN.addEventListener('click', (e) => deleteMovie(e, movie._id));}
   if(editBTN){editBTN.addEventListener('click', (e) => editMovie(e, movie._id));}
